...
| V-136 | REQ-INT-035 | TEST | Publish deterministic external I/O envelopes through the federation bridge and validate logical tick progression, bounded timestamp latency, payload codec integrity, and fail-closed paths (unsupported codec, invalid timing config, non-deterministic envelope rejection). | Bridge emits deterministic event frames for valid inputs and rejects invalid timing/codec/determinism conditions with explicit failures and no partial output. |
| V-137 | REQ-INT-036 | TEST | Validate federation route identity fields (`federate_id`, `route_key`, `route_sequence`) and enforce time-authority controls (required timestamp, max future skew, monotonic source timestamp progression per route, allowlisted source routing). | Frames include stable route identity metadata and reject source timestamp/routing policy violations fail-closed without advancing valid event state. |
| V-138 | REQ-INT-037 | TEST | Execute federation fan-out across multiple endpoints with mixed approved formats and verify endpoint identity metadata (`endpoint_id`, `federate_key_id`) plus per-endpoint route sequencing. | Fan-out produces deterministic per-endpoint frames for valid endpoint sets and rejects invalid endpoint format/policy configuration fail-closed. |
| V-139 | REQ-INT-038; REQ-SEC-014 | TEST | Validate federate trust and key-lifecycle policy enforcement (key validity window, endpoint key allowlist, attestation policy) and assert deterministic audit logging for federation publish/deny decisions. | Frames include trust metadata (`federate_key_epoch`, key validity, attestation tag) for valid policy, and violations are denied fail-closed with auditable `federation_bridge_denied` events containing federate/key/route-or-endpoint context. |
| V-140 | REQ-SYS-022 | TEST | Start runs with valid and invalid mission-thread profile + primary-role declarations (missing, unknown, malformed). | Valid declarations allow startup; invalid declarations fail closed pre-run with explicit denial reason. |
| V-141 | REQ-INT-039 | TEST | Execute degraded-mode denial flows across at least two mission-thread profiles and role presets under deterministic fixtures. | UI guidance text and action ordering are deterministic per role/thread and complete within configured interaction-budget limits. |
| V-142 | REQ-VER-007 | TEST | Run mission-thread verification suite and compute acceptance metrics for false-denial, false-acceptance, operator recovery time, and replay determinism. | All metrics are produced in machine-readable output and meet documented thresholds for the selected mission thread. |
| V-143 | REQ-DOC-002 | INSPECTION | Review release artifact bundle for scope declaration content and references. | Release package includes intended users, constraints, exclusions, and legal/policy boundaries with traceable document references. |
| V-144 | REQ-CM-002 | INSPECTION | Review `.gitignore`, `docs/git_process.md`, and tracked files under `docs/agents_research/`. | Ignore policy enforces private-by-default behavior; only approved control artifacts remain tracked; non-approved research files are untracked or rejected. |
