...
| V-139 | REQ-SEC-014; REQ-INT-038 | TEST | Validate federate trust policy and key lifecycle controls: invalid key windows, expired key material, untrusted endpoint key IDs, and missing required attestation tags; verify lifecycle metadata fields in emitted frames. | Federation bridge rejects trust/key-policy violations fail-closed and emits deterministic lifecycle metadata for accepted frames. |
