...
- REQ-SEC-014: Federation bridge shall enforce federate trust policy controls (federate key allowlists, key-validity windows, and required attestation tags when configured) and audit-log publish/deny decisions with federate key and route/endpoint context using deterministic, fail-closed decision paths.
